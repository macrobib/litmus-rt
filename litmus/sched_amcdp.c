...
/*Abstracts task and its deferment point to be maitaned in oredered heap.*/
typedef struct {
    long dp; /*Deferment point.*/
...
static int amc_dp_find_first_expiry(struct bheap_node* a, struct bheap_node* b){
...
static void amc_update_defer_points(struct task_struct* t){
    ...
    else if(tsk_rt(t)->job_params.dp < active_dp->dp){
        /*An earlier defer point exists compared to active one, reset timer.*/

    }
    else{
        bheap_insert(amc_dp_find_first_expiry, deferred_preemption_heap, tsk_rt(t)->heap_node);
    }
}
