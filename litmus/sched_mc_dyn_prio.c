/* Implementation of mc edf with dynamic priority.
 * kernel/sched_edfdp.c
 *
 */
...
edfdp_domain_t local_domain;

/*EDFVD: storage for the tasks removed from criticality change.*/
static struct bheap edfdp_release_bin[MAX_CRITICALITY_LEVEL];

static void edfdp_domain_init(edfdp_domain_t* edfdp,
			       check_resched_needed_t check,
			       release_jobs_t release,
			       int cpu)
{
	edf_domain_init(&edfdp->domain, check, release);
	edfdp->cpu      		= cpu;
	edfdp->scheduled		= NULL;
}
...
static void lower_system_criticality(void){
    ...
    rt_domain_t* edfdp = &local_domain.domain;

    if(current_criticality > 0){
        current_criticality -= 1;
        release_bin = &edfdp_release_bin[current_criticality];
        update_release_heap(edfdp, release_bin, edf_ready_order, 1);
    }
    else{
...
static int edfdp_check_criticality(struct bheap_node* node){
...
static void add_low_crit_to_wait_queue(struct task_struct* t){
    struct bheap* release_bin = &edfdp_release_bin[current_criticality - 1];
...
static void preempt(edfdp_domain_t *edfdp)
{
	preempt_if_preemptable(edfdp->scheduled, edfdp->cpu);
}
...
{
	unsigned long		flags;
	edfdp_domain_t* 	edfdp = &local_domain;
	lt_t			now;

	raw_spin_lock_irqsave(&edfdp->slock, flags);
	now = litmus_clock();
	...
	if (edfdp->scheduled != t) {
		/* holder may be queued: first stop queue changes */
		raw_spin_lock(&edfdp->domain.release_lock);
		if (is_queued(t) &&
		    ...
		    edf_preemption_needed(&edfdp->domain, edfdp->scheduled))
				preempt(edfdp);
		raw_spin_unlock(&edfdp->domain.release_lock);
	} /* else: nothing to do since the job is not queued while scheduled */

	raw_spin_unlock_irqrestore(&edfdp->slock, flags);
}
...
static void unboost_priority(struct task_struct* t)
{
	 ...
	 * care about it. */
	BUG_ON(edfdp->scheduled != t && is_realtime(t));
	...
	if (edf_preemption_needed(&edfdp->domain, edfdp->scheduled))
		preempt(edfdp);
...
static int edfdp_preempt_check(edfdp_domain_t *edfdp)
{
	if (edf_preemption_needed(&edfdp->domain, edfdp->scheduled)) {
		preempt(edfdp);
		return 1;
 ...
 */
static int edfdp_check_resched(rt_domain_t *edf)
{
	edfdp_domain_t *edfdp = container_of(edf, edfdp_domain_t, domain);

	/* because this is a callback from rt_domain_t we already hold
	 * the necessary lock for the ready queue
	 */
	return edfdp_preempt_check(edfdp);
}
...
static struct task_struct* edfdp_schedule(struct task_struct * prev)
{
	edfdp_domain_t* 	edfdp = &local_domain;
	rt_domain_t*		edf  = &edfdp->domain;
	struct task_struct*	next;
    
	int 			out_of_time, sleep, preempt,
				np, exists, blocks, resched;
   
	raw_spin_lock(&edfdp->slock);
	/* sanity checking
	 * differently from gedf, when a task exits (dead)
	 * edfdp->schedule may be null and prev _is_ realtime
	 */
	BUG_ON(edfdp->scheduled && edfdp->scheduled != prev);
	BUG_ON(edfdp->scheduled && !is_realtime(prev));
    
	/* (0) Determine state */
	exists      = edfdp->scheduled != NULL;
	blocks      = exists && !is_current_running();
	out_of_time = exists && budget_enforced(edfdp->scheduled)
			     && budget_exhausted(edfdp->scheduled);
	np 	    = exists && is_np(edfdp->scheduled);
	sleep	    = exists && is_completed(edfdp->scheduled);
	preempt     = edf_preemption_needed(edf, prev);

    if(exists && !budget_precisely_enforced(edfdp->scheduled))
        BUG_ON(exists);
    ...
    if(!np && (out_of_time ||sleep)){
        ...
        if(out_of_time){
            ...
            if(is_task_high_crit(edfdp->scheduled)){
                ...
                if(raise_system_criticality()){
                    ...
                    if(has_control_page(edfdp->scheduled)){
                        struct control_page* cp  = get_control_page(edfdp->scheduled);
                        cp->active_crit = current_criticality;
                    }
                    if(replenish_task_for_mode(edfdp->scheduled, eRAISE_CRIT)){
                        resched = 0;
                        exists = 1;
                    }
                    else{
                    ...
                    job_completion(edfdp->scheduled, !sleep);
                    resched = 1;
                }
            }
            else{
	...
	if (np && (out_of_time || preempt || sleep)){
		request_exit_np(edfdp->scheduled);
        BUG_ON(1);
        }
	...
	if ((!np || blocks) && (resched || !exists)) {
        ...
        if (exists && !blocks){
            ...
            if(is_task_eligible(edfdp->scheduled)){
                /*If completed task is eligible, then requeue, else store.*/
			        requeue(edfdp->scheduled, edf);
                }
                else{
                    add_low_crit_to_wait_queue(edfdp->scheduled);
                }
            }
   ...
   /** if(next){
        ...
        if(next == edfdp->scheduled)
            BUG_ON(budget_exhausted(next));
    }**/
	edfdp->scheduled = next;
	sched_state_task_picked();
	raw_spin_unlock(&edfdp->slock);
	return next;
}
 ...
 */
static void edfdp_task_new(struct task_struct * t, int on_rq, int is_scheduled)
{
	...
	if (is_scheduled) {
		...
		if (on_rq) {
			...
			edfdp_preempt_check(edfdp);
		}
	}
	raw_spin_unlock_irqrestore(&edfdp->slock, flags);
}

static void edfdp_task_wake_up(struct task_struct *task)
{
	unsigned long		flags;
	edfdp_domain_t* 	edfdp = &local_domain;
	rt_domain_t* 		edf  = &local_domain.domain;
...
static void edfdp_task_block(struct task_struct *t)
{
...
static void edfdp_task_exit(struct task_struct * t)
{
	unsigned long flags;
	edfdp_domain_t* 	edfdp = &local_domain;
	rt_domain_t*		edf;
...
static unsigned int edfdp_get_srp_prio(struct task_struct* t)
{
...
int edfdp_fmlp_lock(struct litmus_lock* l)
{
...
int edfdp_fmlp_unlock(struct litmus_lock* l)
{
...
int edfdp_fmlp_close(struct litmus_lock* l)
{
	struct task_struct *t = current;
	struct fmlp_semaphore *sem = fmlp_from_lock(l);
	unsigned long flags;
...
void edfdp_fmlp_free(struct litmus_lock* lock)
{
	kfree(fmlp_from_lock(lock));
}

static struct litmus_lock_ops edfdp_fmlp_lock_ops = {
	.close  = edfdp_fmlp_close,
	.lock   = edfdp_fmlp_lock,
	.unlock = edfdp_fmlp_unlock,
	.deallocate = edfdp_fmlp_free,
};

static struct litmus_lock* edfdp_new_fmlp(void)
{
	...
	sem->litmus_lock.ops = &edfdp_fmlp_lock_ops;
...
static long edfdp_allocate_lock(struct litmus_lock **lock, int type,
				 void* __user unused)
{
	int err = -ENXIO;
...
static struct domain_proc_info edfdp_domain_proc_info;
static long edfdp_get_domain_proc_info(struct domain_proc_info **ret)
{
	*ret = &edfdp_domain_proc_info;
	return 0;
}

static void edfdp_setup_domain_proc(void)
{
	int i, cpu;
	int release_master =
	...
	memset(&edfdp_domain_proc_info, 0, sizeof(edfdp_domain_proc_info));
	init_domain_proc_info(&edfdp_domain_proc_info, num_rt_cpus, num_rt_cpus);
	edfdp_domain_proc_info.num_cpus = num_rt_cpus;
	edfdp_domain_proc_info.num_domains = num_rt_cpus;

	for (cpu = 0, i = 0; cpu < num_online_cpus(); ++cpu) {
		if (cpu == release_master)
			continue;
		cpu_map = &edfdp_domain_proc_info.cpu_to_domains[i];
		domain_map = &edfdp_domain_proc_info.domain_to_cpus[i];
...
static long edfdp_activate_plugin(void)
{
#ifdef CONFIG_RELEASE_MASTER
	int cpu;
...
#ifdef CONFIG_LITMUS_LOCKING
	get_srp_prio = edfdp_get_srp_prio;
#endif

	edfdp_setup_domain_proc();

	return 0;
}

static long edfdp_deactivate_plugin(void)
{
	destroy_domain_proc_info(&edfdp_domain_proc_info);
	return 0;
}

static long edfdp_admit_task(struct task_struct* tsk)
{
...
static struct sched_plugin psn_edf_plugin __cacheline_aligned_in_smp = {
	.plugin_name		= "EDFVD",
	.task_new		= edfdp_task_new,
	.complete_job		= complete_job,
	.task_exit		= edfdp_task_exit,
	.schedule		= edfdp_schedule,
	.task_wake_up		= edfdp_task_wake_up,
	.task_block		= edfdp_task_block,
	.admit_task		= edfdp_admit_task,
	.activate_plugin	= edfdp_activate_plugin,
	.deactivate_plugin	= edfdp_deactivate_plugin,
	.get_domain_proc_info	= edfdp_get_domain_proc_info,
#ifdef CONFIG_LITMUS_LOCKING
	.allocate_lock		= edfdp_allocate_lock,
#endif
};


static int __init init_edfdp(void)
{
    /*Register the schedule domain, still using vestige of pedf scheduler.*/
		...
		edfdp_domain_init(&local_domain,
				   edfdp_check_resched,NULL, 0);
	return register_sched_plugin(&psn_edf_plugin);
}

module_init(init_edfdp);
